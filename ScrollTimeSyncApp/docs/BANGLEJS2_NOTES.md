...
#### 4-Byte Record Encoding

Each 10-minute health reading is packed into 4 bytes:
...
Byte 3: movement (uint8, accelerometer delta 0-255)
```
...
Movement is the delta from the Bangle.js accelerometer's cumulative `info.movement` counter, captured each 10-minute health event. Low values (~0-20) indicate stillness/deep sleep; high values (>80) indicate active movement.

Records are stored sequentially starting at `hist[0]`. Record N is at byte offset `N * 4`. The timestamp of record N is `histStart + N * 600` (each record is 10 minutes = 600 seconds).
...
- Data: `histLen × 4` bytes (max 600 bytes for 150 entries)
- **Max total: 616 bytes** — ~31 BLE packets at 20-byte MTU, streams in under 1 second
...
3. Read from the TX characteristic (`6e400003-...`) notifications:
   a. First 16 bytes = header. Parse `histLen` at offset 4 (uint16 LE).
